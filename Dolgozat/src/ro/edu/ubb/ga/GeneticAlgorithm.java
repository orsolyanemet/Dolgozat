...
public class GeneticAlgorithm {
	...
	private static int drawEveryX;
	private static int maxRoomMutationIteration;
	...
	private static void getParameters() throws IOException {
		...
		try (InputStream input = new FileInputStream("resources/config")) {
			...
			drawEveryX = Integer.parseInt(properties.getProperty("drawEveryX"));
			maxRoomMutationIteration = Integer.parseInt(properties.getProperty("maxRoomMutationIteration"));
	...
	private static ArrayList<ArrayList<Object>> roomMutation(ArrayList<ArrayList<Object>> chromosome) {
		...
		for (int i = 0; i < chromosome.size(); i++) {
			...
			if (toMutate > pmr) {
				...
				boolean found = false;
				int iter = 0;
				while (!found && iter < maxRoomMutationIteration) {
				...
				if (iter >= maxRoomMutationIteration) {
					gene = chromosome.get(i);
				}
			} else {
				gene = chromosome.get(i);
	...
	private static int findMaxFitness() {
		int max = 0;
		for (int i = 0; i < fitnessOfChromosomes.length; i++) {
			if (max < fitnessOfChromosomes[i])
				max = fitnessOfChromosomes[i];
		}
		return max;
	}
	...
	private static int selection() {
		...
		int partialSum = 0;
		int maxFitness = findMaxFitness();
		for (int i = 0; i < populationSize; i++) {
			partialSum += maxFitness - fitnessOfChromosomes[i];
	...
	private static int[] foundFitChromosome() {
		fitnessCalculation();
		...
		return new int[] { fitnessValue, fitnessValueIndex };
	}

	private static void printFitChromosome() {
		int[] result = foundFitChromosome();
		System.out.println("Az eddigi legfittebb kromoszoma:");
		System.out.println("Utkozesek:" + result[0]);
		int chromosomeSize = population.get(0).size();
		for (int i = 0; i < chromosomeSize; i++) {
			System.out.println(population.get(result[1]).get(i));
		}
	}

	private static void drawOnChart(double[] xData, double[] yData) {
		XYChart chart = QuickChart.getChart("Genetic Algorithm", "Generation", "Fitness", "fitness", xData, yData);
	...
	private static void geneticAlgorithm() {
		...
		found = false;
		int[] result;
		double[] xData = new double[(maxIteration / drawEveryX) + 1];
		double[] yData = new double[(maxIteration / drawEveryX) + 1];
		int j = 0;
		while (iter < maxIteration && !found) { // Fitness fuggveny kiertekelese a populaciora
			fitnessCalculation();
			i = 0;
			while (i < populationSize && !found) {
				if (countCollisions(population.get(i)) == 0) {
					found = true;
					printFittestChromosome(i);
				}
				i++;
			}
			if (iter % drawEveryX == 0 && maxIteration >= drawEveryX) {
				result = foundFitChromosome();
				xData[j] = iter;
				yData[j++] = result[0];
			}
		...
		xData[j] = iter;
		yData[j] = result[0];
		drawOnChart(Arrays.copyOfRange(xData, 0, j + 1), Arrays.copyOfRange(yData, 0, j + 1));
		if (!found)
			printFitChromosome();
	}
